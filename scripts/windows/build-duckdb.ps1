...
tarball, and builds the static library targets with Visual Studio 2022. The
resulting COFF .lib files are placed under:

  <repo>/.cache/duckdb-windows/build/Release
...
$releaseLibDir = Join-Path $buildRoot "Release"

if (-not $Rebuild -and (Test-Path (Join-Path $releaseLibDir "duckdb_static.lib"))) {
    Write-Host "DuckDB static libraries already built: $releaseLibDir"
    return
}

New-Item -ItemType Directory -Force -Path $cacheRoot, $srcRoot, $releaseLibDir | Out-Null
...
# 3. Configure with Visual Studio 2022 (x64, Release, dynamic CRT to match PHP).
...
if (-not (Test-Path (Join-Path $buildRoot "CMakeCache.txt"))) {
    Write-Host "Configuring DuckDB build (MSVC x64, Release)"
    $configureArgs = @(
        "-S", $srcRoot,
        "-B", $buildRoot,
...
#    extension. All are COFF .lib archives under $releaseLibDir.
$targets = @(
    "duckdb_static",
...
& cmake --build $buildRoot --config Release --target $targets --parallel
if ($LASTEXITCODE -ne 0) { throw "DuckDB CMake build failed" }

# 5. Verify every expected library exists.
$expectedLibs = @(
    "duckdb_static.lib",
...
foreach ($libName in $expectedLibs) {
    $libPath = Join-Path $releaseLibDir $libName
    if (-not (Test-Path $libPath)) {
        throw "expected DuckDB library missing: $libPath"
    }
}

$totalBytes = (Get-ChildItem $releaseLibDir -Filter "*.lib" | Measure-Object -Property Length -Sum).Sum
Write-Host "DuckDB static libraries built: $releaseLibDir ($([math]::Round($totalBytes / 1MB, 1)) MB)"
